    ...
    if len(taking_pieces) !=0:
        ...
        for l in taking_pieces:
            if  table.tabla[l[0]-1][l[1]-1].material < lowest_material:
                lowest_material = table.tabla[l[0]-1][l[1]-1].material
                small_material = type(table.tabla[l[0]-1][l[1]-1])
...
def evaluation(table):
    ...
    if True:
        ...
        eval_bar += (len(all_possible_moves) - len(all_possible_moves_copy))/20
    return eval_bar
...
def best_move_advanced(table, oldal):
    ...
    for n in active_pieces(table, oldal):
        for m in possible_moves(table,(n)):
            all_possible_moves.add((n,m))
    calc_tabla = copy.deepcopy(table)
    best_move_evalv = float()
    if oldal:
        best_move_evalv = (best_move_better(maintabla,oldal)[0], evaluation(maintabla)+best_move_better(maintabla,oldal)[1])
    else:
        best_move_evalv = (best_move_better(maintabla,oldal)[0], evaluation(maintabla)-best_move_better(maintabla,oldal)[1])
    list_evalv = set()
    for move in all_possible_moves:
        ...
        if sakkmatt(calc_tabla, not oldal):
            ...
            move_sakknak(calc_tabla,best_move(calc_tabla, not oldal)[0], best_move(table, not oldal)[1] )
            if sakkmatt(calc_tabla, oldal):
                continue
            else:
                        move_sakknak(calc_tabla,best_move(calc_tabla, oldal)[0], best_move(table, oldal)[1] )
                        if sakkmatt(calc_tabla, not oldal):
                            return move
                        else:
                            move_sakknak(calc_tabla,best_move(calc_tabla, not oldal)[0], best_move(table, not oldal)[1] )
